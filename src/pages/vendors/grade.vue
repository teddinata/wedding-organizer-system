...
const fetchVendorGradeList = () => {
  axios
    ...
    .then((response) => {
      gradeList.value = response.data.data;
      totalGrade.value = response.data.data.total as number;
...
onMounted(() => fetchVendorGradeList());

// Create and edit vendor grade
...
const gradeData = ref()
const payload = ref<{ name: string | null; description: string | null }>({
  name: "",
  description: "",
});
  ...
  refVForm.value?.validate().then(({ valid: isValid }) => {
    if (isValid) {
      if (gradeData.value === undefined) {
        ...
        axios
          .put(`/vendor-grade/${gradeData.value.id}`, {
            name: payload.value.name,
            description: payload.value.description,
...
const deleteGrade = () => {
  isLoading.value = true;
  axios
    .delete(`/vendor-grade/${gradeData.value.id}`)
    .then((response) => {
      isLoading.value = false;
      flashMessage.value.message = response.data.message;
      flashMessage.value.type = "error";
      ...
      if (gradeList.value.data.length <= 1) toPage.value -= 1
      fetchVendorGradeList();
    })
    .catch((e) => {
      isLoading.value = false;
...
<template>
  <VRow class="match-height">
          ...
          <VRow justify="space-between">
            ...
            <VCol cols="12" md="6" sm="6">
              <VRow>
                ...
                <VCol cols="4">
                  <VBtn @click="isDialogVisible = !isDialogVisible">Add Grade</VBtn>
                </VCol>
              </VRow>
            </VCol>
          </VRow>
          <div class="mt-5" v-if="gradeList">
            <VDivider />
            <VDataTable :headers="gradeListHeaders" hide-default-footer :search="search" :items="gradeList.data"
              ...
              <template #bottom>
                ...
                <div class="d-flex align-center justify-space-between flex-wrap gap-3 pa-5 pt-3">
                  <p class="text-sm text-disabled mb-0">
                    {{ paginationMeta(options as Options, totalGrade) }}
                  </p>
                  <VPagination v-model="options.current_page"
                    :total-visible="Math.min(Math.ceil(totalGrade / options.per_page), options.last_page)"
                    :length="Math.min(Math.ceil(totalGrade / options.per_page), options.last_page)"
    ...
    <MainDialog v-model:is-dialog-visible="isDialogVisible" @update:is-dialog-visible="(val) => {
      ...
      ">
      <div class="mx-auto">
        <h2 class="text-center">{{ gradeData ? "Edit" : "Add" }} Grade</h2>
        <FlashMessage v-model="isAlertDialogVisible" :message="flashMessage.message" :type="flashMessage.type"
          variant="tonal" class="mt-4" />
        <VForm ref="refVForm" @submit.prevent="submitGrade()">
          <VRow class="mt-3">
            <VCol cols="12">
              <AppTextField v-model="payload.name" label="Name" type="text"
                :rules="[requiredValidator, stringLengthBetweenValidator(payload.name as string, 3, 15), alphaValidator]"
                placeholder="Ex. Grade A" />
            </VCol>
            <VCol cols="12">
              <AppTextField v-model="payload.description" label="Description" type="text"
                :rules="[stringLengthBetweenValidator(payload.description as string, 5, 25)]" placeholder="Description" />
            </VCol>
    ...
    <!-- Delete Grade Dialog -->
    <MainDialog v-model:is-dialog-visible="isDeleteDialogVisible" title="Delete Grade" @update:is-dialog-visible="(val) => {
      if (!val) {
        gradeData = undefined
        payload = {
          description: null,
      ...
      Are you sure want to delete grade
      <span class="font-weight-bold">{{ gradeData && gradeData.name }}</span>?
      <VRow justify="end">
        <VCol cols="12">
          <div class="d-flex align-center justify-end flex-wrap py-5">
            ...
            <VBtn :loading="isLoading" :disabled="isLoading" color="error" @click="deleteGrade()">
              Delete</VBtn>
          </div>
        </VCol>
      </VRow>
